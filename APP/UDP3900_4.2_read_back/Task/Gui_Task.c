...
const osThreadAttr_t GUITask_attributes = {
    .name = "GUITask",
    .stack_size = 1024 * 8,          // LVGL 比较吃内存，建议 H7 给 32KB (8*4)
    .priority = (osPriority_t) osPriorityNormal,
};
lv_ui ui;

static osThreadId_t uiRefreshTaskHandle;

// 定义 UI 刷新任务属性：优先级设为 Normal，略低于 CAN 任务
static const osThreadAttr_t uiRefreshTask_attr = {
    .name = "UIRefresh",
    .stack_size = 512 * 4,
    .priority = (osPriority_t) osPriorityNormal,
};
...
void GUI_Task(void *argument)
{
  /* USER CODE BEGIN GUI_Task */    
    ...
    //
    uiRefreshTaskHandle = osThreadNew(UI_Refresh_Task, NULL, &uiRefreshTask_attr);
    for(;;) {
       lv_timer_handler();
        osDelay(5);
