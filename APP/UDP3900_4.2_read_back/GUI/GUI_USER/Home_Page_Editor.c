    ...
    switch(code)
    {
        ...
        case LV_EVENT_KEY: {
            ...
            switch(key) {
                case LV_KEY_UP:
                    if(is_neg) 
                        current_val -= cfg->steps[*index_ptr]; // 负向通道：UP 键让数值更负（绝对值变大）
                    else 
                        current_val += cfg->steps[*index_ptr]; // 正向通道：UP 键让数值更正
                    break;
                    
                case LV_KEY_DOWN:
                    if(is_neg) 
                        current_val += cfg->steps[*index_ptr]; // 负向通道：DOWN 键向 0 靠近
                    else 
                        current_val -= cfg->steps[*index_ptr]; // 正向通道：DOWN 键减小
                    break;
            ...
            // --- 边界限制保持不变 ---
            if(is_neg && current_val > 0.0) current_val = 0.0; // 负向通道不能过 0 变成正数
            ...
            cfg->set_val(current_val); // 更新硬件
            
            // 刷新显示：直接传真实值，不再用 fabs()，让系统自动印出负号
            Set_RefreshUI(obj, index_ptr, cfg->fmt, current_val);
            break;
        }
...
void volSet_handler(lv_event_t * e) {
    static uint8_t idx = DEFAULT_HOME_INDEX; 
    NumberInput_Process(6, _set_h_v, (&CFG_H_VOLT), 0); // is_neg = 0
    home_page_item_logic_core(e, &idx, &CFG_H_VOLT, false);
}

// 电流：正向
void ampSet_handler(lv_event_t * e) {
    static uint8_t idx = DEFAULT_HOME_INDEX;
    NumberInput_Process(6, _set_h_i, (&CFG_H_CURR), 0); // is_neg = 0
    home_page_item_logic_core(e, &idx, &CFG_H_CURR, false);
}
...
void ampSet_handler_neg(lv_event_t * e) {
    static uint8_t idx = DEFAULT_HOME_INDEX;
    // --- 关键：这里传 1 ---
    NumberInput_Process(6, _set_h_i_neg, (&CFG_H_CURR_NEG), 1); // is_neg = 1
    home_page_item_logic_core(e, &idx, &CFG_H_CURR_NEG, true);
}
